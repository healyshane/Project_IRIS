...
# A function to loop over data set range and depending on Species
# will print out the minimium, maximum, mean and standard deviation. 
# Species is 0 for Iris-setosa, 1 for Iris-versicolor, 2 for Iris-virginica, 3 for all species.
# Feature is SL for Sepal Length, SW for Sepal Width
# PL for Petal Length and PW for Petal Width. 
...
def Iris(species,feature):
    ...
    for i in range (len(SPEC)):
        if SPEC[i] == species:
            ...
            p = np.array([results])
    return p
# To allow a user to input a selection.
...
print("")
#Print the statistical information on species of Iris by calling function Iris. 
